...
This example shows how to use the AESCBC driver. The target expects the user
...
to the user. '1' is to select 16 byte plaintext and '2' is for 128 byte
plaintext. The user can interact with the target until the application is ended.
...
It performs an encryption on a predefined message using `AESCBC_oneStepEncrypt`
and displays the encrypted message to the console. Then, it performs a decryption
using `AESCBC_oneStepDecrypt` on the encrypted message. Decrypted message is also
