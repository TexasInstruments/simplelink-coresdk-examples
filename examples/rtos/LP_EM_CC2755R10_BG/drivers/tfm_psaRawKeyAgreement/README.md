...
This TF-M example demonstrates using the PSA Crypto API to import private keys
with various lifetimes and perform an Elliptic-Curve Diffie-Hellman (ECDH) key
...
#### Steps to run the example with CCS:
  ...
  6. To restore NS application symbols: Select *Run* -> *Load* -> *Add Symbols*.
   Select the NS application `tfm_psaRawKeyAgreement_XXXX.out`.
